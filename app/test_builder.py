...
page, content = page_extractor.extract(page_data)


attachment_data = client.get_attachments(page_id)


attachments = attachment_extractor.extract(page_id, attachment_data)


print("Attachments extracted:", len(attachments))


attachment_repo.save_many(attachments)


knowledge_page = builder.build(
    page_data, content.model_dump(), [a.model_dump() for a in attachments]
)


knowledge_repo.save(knowledge_page)


print("Knowledge pages:", knowledge_repo.count())
