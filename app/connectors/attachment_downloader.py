...
class AttachmentDownloader:
    ...
    def download(self, attachment):
        ...
        url = self.base_url + attachment.download_url

        file_path = self.download_dir / attachment.filename

        logger.info(f"Downloading {attachment.filename}")

        with httpx.stream("GET", url, auth=self.auth, timeout=120) as response:
