...
class DeltaCrawler:
    ...
    def run(self):
        ...
        checkpoint = self.checkpoint_repo.get("DPCC")

        if checkpoint:

            last_sync = checkpoint["last_sync_time"]

        else:

            last_sync = "2000-01-01 00:00"

        logger.info(f"Delta sync from {last_sync}")

        pages = self.client.get_pages_modified_after(last_sync)

        results = pages.get("results", [])

        logger.info(f"Changed pages: {len(results)}")
